...
Cleaning the table by converting tmax, tmin and year into numeric type and seperate date into 3 columns(year,month,day). tmax, tmin and prcp are set to normal unit for us to read (degree of C and mm).
```{r}
ny_noaa_clean <- 
  ny_noaa %>%
...
```
The plot of average tmax over years for all stations are plot by `geom_line()`. Each color line represents a different station. Even though there is few outliers (14°C in July or -15°C in January, etc.) exist, the average max temperature in January and July constantly fluctuates in their certain range. As the graph shown, the average max temperature of July is overall higher than January, and January's average max temperature fluctuates in a larger range than July.
```{r}
ny_noaa_clean %>%
